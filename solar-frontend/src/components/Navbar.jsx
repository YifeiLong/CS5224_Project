...
const Navbar = ({ onOpenPayment, onOpenLogin, activeTab, onTabChange }) => {
  ...
  return (
    <AppBar position="static" elevation={1} sx={{ backgroundColor: '#ffffff', color: '#1e293b' }}>
      <Toolbar disableGutters sx={{ px: 3 }}>
        ...
        <Box onClick={() => onTabChange('Features')}
          ...
          <SunIcon />
          <Typography variant="h6" sx={{ fontWeight: 800, letterSpacing: '-0.5px', whiteSpace: 'nowrap' }}>
        ...
        <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: '4px', flexGrow: 1, justifyContent: 'center' }}>
          {NAV_TABS.map((tab) => (
            <Button key={tab} onClick={() => onTabChange(tab)} sx={{
              ...
              color: activeTab === tab ? '#f59e0b' : '#1e293b',
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
          {currentUser ? (
            ...
            <ClickAwayListener onClickAway={() => setDropdownOpen(false)}>
              <Box sx={{ position: 'relative' }}>
                <Button
                  ...
                  sx={{
                    textTransform: 'none', color: '#1e293b', fontWeight: 600,
            ...
            <Button onClick={onOpenLogin} sx={{ textTransform: 'none', color: '#64748b' }}>
              Login
            </Button>
          )}

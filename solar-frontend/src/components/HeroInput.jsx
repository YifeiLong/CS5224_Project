...
const HeroInput = () => {
  const { setSolarData, solarData } = useUser();
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ position: 'relative', zIndex: 2, textAlign: 'center', px: 2, width: '100%', maxWidth: '800px' }}>
        ...
        <Typography variant="h6" sx={{ color: '#cbd5e1', mb: 3, fontWeight: 400 }}>
          Get instant ROI analysis. Know about Solar Energy, transition to Solar PV System today!
        </Typography>
            ...
            <Box sx={{ textAlign: 'left' }}>
              ...
              <ToggleButtonGroup
                ...
                onChange={(e, newType) => {
                  if (newType !== null) {
                    setUserType(newType);
                    if (solarData) setPostalError('Property type changed — click Analyze Now to update results.');
                  }
                }}
                fullWidth
                sx={{

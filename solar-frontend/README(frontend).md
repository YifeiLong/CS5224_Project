...
Create a `.env` file in `solar-frontend/` for local backend:
```
...
│   ├── index.html
│   └── assets/
│       ├── index-DicA3_HH.js         # Bundled JS (all React components)
...
└── src/
    ...
    │
    ├── assets/
    │   ├── solar-panel-on-a-red-roof-reflecting-the-sun-web.jpg
    │   └── SEI_270344931.webp
    │
    ├── context/
    ...
    │   ├── HeroInput.jsx             # Property input form
...
### `.env.production`
Contains the production backend URL, automatically used by Vite when running `npm run build`:
```
VITE_API_BASE_URL=https://solar-backend-940317004552.us-central1.run.app
```
This points to the FastAPI backend deployed on **Google Cloud Run**. Do not commit this file with a different URL unless the backend has changed.

### `firebase.json`
Configures Firebase Hosting to serve the `dist/` folder:
```json
{
  "hosting": {
    "public": "dist",
...
```
The `rewrites` rule sends all routes to `index.html` — required for React single-page app routing.

### `.firebaserc`
Binds this directory to the Firebase project `solar-yield-app`:
```json
{ "projects": { "default": "solar-yield-app" } }
```

### `dist/`
The compiled static output generated by `npm run build`. This is what Firebase Hosting serves to users. Do not edit files in `dist/` directly — regenerate by running `npm run build`.
...
The public URL remains the same after every deploy.
...
| `solarData` | object / null | Transformed forecast result after `/forecast` call |
...
4. Shows the actual backend error message if the request fails
...
- **Production**: reads `VITE_API_BASE_URL` from `.env.production` → `https://solar-backend-940317004552.us-central1.run.app`
...
```js
{
  ...
  roof_area_m2:       4.645,         // converted from sqft (× 0.092903)
  ...
  monthly_load_kwh:   null,          // backend assumes 80% self-consumption
  tariff_sgd_per_kwh: null,          // backend uses Prophet ML tariff forecast
}
```

The raw response is cached in `_rawForecastResult` immediately for use by `/advisory`.

### `/advisory` Request
```js
{
  user_question:   "what is the payback period?",
  forecast_result: _rawForecastResult,  // raw /forecast response — NOT the UI-transformed shape
  language:        "en",
  run_mode:        "local",             // uses DeepSeek-R1-Distill-Qwen-1.5B on Cloud Run
  max_new_tokens:  220,
  temperature:     0.3,
  top_p:           0.9,
}
...
`run_mode: "local"` means the backend runs the downloaded DeepSeek LLM for inference.
The raw `/forecast` response is forwarded verbatim because `llm_service.py` needs the original backend keys (`pv_kwh.neutral`, `roi.payback_years`, `tariff_series`, etc.).
...
| Feature | Not logged in | Logged in (Free) | Pro |
|---|---|---|---|
| Run forecast analysis | ✅ | ✅ | ✅ |
| Historical stat cards + charts | ✅ | ✅ | ✅ |
| Pro stat cards + charts | ❌ blurred | ❌ blurred | ✅ |
| AI chat advisory | ❌ | ❌ | ✅ |

## AI Acknowledgement
We acknowledge the use of Claude AI to guide and generate front-end codes for better visualization and in a structured way based on our ideas, and we are responsible for its result and correctness

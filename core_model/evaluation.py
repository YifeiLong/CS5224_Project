...
def load_and_predict_sunshine(months=12):
    ...
    future_dates = model_rain.make_future_dataframe(periods=months, freq='MS')
    ...
    df_sun = forecast_sun[['ds', 'yhat']].tail(months).rename(columns={'yhat': 'H_Sunshine_Hrs'})
    return df_sun.reset_index(drop=True)


def load_and_predict_tariff(user_type='domestic', months=12):
    filename = f'./model/tariff_{user_type}.json'
    with open(filename, 'r') as fin:
        model_tariff = model_from_json(json.load(fin))

    future_dates = model_tariff.make_future_dataframe(periods=months, freq='MS')
    forecast_tariff = model_tariff.predict(future_dates)

    df_tariff = forecast_tariff[['ds', 'yhat']].tail(months).rename(columns={'yhat': 'Tariff_Cents_per_kWh'})
    return df_tariff.reset_index(drop=True)


def calculate_solar_roi(roof_area_m2, user_type='domestic', months_to_predict=12):
    df_sun = load_and_predict_sunshine(months_to_predict)
    df_tariff = load_and_predict_tariff(user_type, months_to_predict)
    ...
    final_cols = ['Date', 'H_Sunshine_Hrs', 'D_Days', 'E_month_kWh', 'Tariff_Cents_per_kWh', 'Savings_Dollars',
                  'Cumulative_Savings']
...
if __name__ == "__main__":
    ...
    total_yield = roi_df['E_month_kWh'].sum()
    total_savings = roi_df['Savings_Dollars'].sum()
    print(f"\nYearly Yield: {total_yield:.2f} kWh")
    print(f"Yearly Savings: ${total_savings:.2f}")
